...
$config->blocklist('scanner-ua', function (ServerRequestInterface $req) use ($scannerUserAgents): bool {
    ...
    foreach ($scannerUserAgents as $scanner) {
...
$config->blocklist('empty-ua', function (ServerRequestInterface $req): bool {
...
$config->blocklist('scanner-paths', function (ServerRequestInterface $req) use ($scannerPaths): bool {
    ...
    if (preg_match('/\.(bak|backup|old|orig|save|swp|~)$/i', $path)) {
        return true;
    }
...
$config->blocklist('suspicious-patterns', function (ServerRequestInterface $req): bool {
    $uri = $req->getUri()->getPath() . '?' . $req->getUri()->getQuery();
...
for ($i = 1; $i <= 35; $i++) {
    if ($i <= 3 || $i >= 29) {
        $testRequest("Rapid request $i from $ip", '/api/data', ['User-Agent' => 'Mozilla/5.0'], $ip);
    } elseif ($i === 4) {
