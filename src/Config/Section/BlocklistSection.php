...
final class BlocklistSection
{
     ...
     *
     * @param list<string>|null $requiredHeaders Headers that must be present. `null` uses the curated
     *                                           SuspiciousHeadersMatcher::DEFAULT_REQUIRED_HEADERS; a non-empty list
     *                                           overrides them; an explicit empty list (`[]`) requires no headers, so
     *                                           the rule never matches (effectively disabled).
     */
