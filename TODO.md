...
- [ ] Extract shared fail2ban check/increment/ban/dispatch logic into a private method used by both `decide()` and `processRecordedFailure()`
- [ ] Remove duplicated increment logic between `Firewall::increment()` and `FixedWindowStrategy`
- [ ] Extract `CoreRule::evaluateOperator()` and `collectVariableValues()` into strategy classes
- [ ] Response-based fail2ban counting via RequestContext (inspect response after handler)
...
- [ ] `DiagnosticsCounters` implements `EventDispatcherInterface` — semantic mismatch, should wrap a real dispatcher
...
- [ ] Make X-Phirewall/X-Phirewall-Matched response headers opt-in (disabled by default) — breaking change
...
### Documentation
- [ ] Add "How It Works" architectural flow diagram (request → track → safelist → ... → allow2ban)
- [ ] Add OWASP false positives guidance and rule tuning
- [ ] Add rate limiting limitations/defense-in-depth section
- [ ] Add access control warning to metrics endpoint example
- [ ] Document phpredis/ext-redis support (or lack thereof)
- [ ] Add PHP/dependency compatibility matrix to docs
