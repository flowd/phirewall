...
// Track rules are passive counters evaluated during decide() — they inspect
// request headers (set by upstream middleware or proxies), unlike Fail2Ban
// failures which are signaled post-handler via RequestContext::recordFailure().
